            ...
            >
                ...
                <form @submit.prevent="submitForm" class="p-8">
                    <transition name="fade-slide" mode="out-in">
                            ...
                            <div class="mb-10 space-y-8">
                                <div>
                                    ...
                                    >
                                        <div>
                                            ...
                                            >
                                                ...
                                                <option value="Married">
                                                    Single Parent with Partial
                                                    Custody (ie. Both Primary
                                                    Responsible Party &
                                        ...
                                        <div
                                            class="rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm font-medium text-blue-800"
                                        >
                                            <strong>Notice:</strong>
                                            Parent/Guardian details, Primary
                                            Responsible Party, and Secondary
                                            Responsible Party details are
                                            required since the patient is not
                        ...
                        <!-- Step 3: Dental History (UPDATED to show Date input always) -->
                            ...
                            <div class="mb-10">
                                ...
                                <div class="mt-8 space-y-4">
                                    ...
                                    >
                                        ...
                                        <!-- Right side: Conditional Text or Date Inputs -->
                                        <div>
                                            <transition name="fade-slide">
                                                <div
                                                    v-if="
                        ...
                        <!-- Step 5: Consents (UPDATED to use Vue3Signature & current date label) -->
                        <div v-else-if="currentStep === 5" key="step5">
                            <div class="mb-10 space-y-10">
                                ...
                                >
                                    ...
                                    <div class="p-6">
                                        <div
                                            class="custom-scrollbar mb-6 h-32 overflow-y-auto rounded-lg border border-slate-200 bg-slate-50 p-4 pr-2 text-sm text-slate-600"
                                        >
                                            <p>
                                            ...
                                            <p class="mt-2">
                                                ...
                                                obligations before agreeing to
                                        ...
                                        >
                                            ...
                                            INFORMATION ABOVE & CONSENT
                                        </p>

                                        <!-- UPDATED: Signature canvas & date signed display -->
                                        <div
...
// UPDATED: Ref collection for multiple consent signatures
...
const clear = () => {
    if (signature.value) {
        console.log(signature.value.isEmpty());
...
// NEW: Helper methods to manage individual consent form signature components
...
const formData: any = reactive({
    ...
    // Step 5
    consents: {
        financialConsent1: { signature: '', date: today },
        informedConsent: { signature: '', date: today },
        financialConsent2: { signature: '', date: today },
        appointmentPolicy: { signature: '', date: today },
    },
});
...
const submitForm = () => {
    ...
    // Save Dental signature data before submitting
    if (signature.value && !signature.value.isEmpty()) {
        const base64 = signature.value.save('image/png');
        formData.dental.signature = dataURLtoFile(
            base64,
            'dental_signature.png',
        );
    } else {
        formData.dental.signature = null; // Ensure empty values don't fail backend validation
    }

    // NEW: Save Consent Form signature images before submitting
    consentSections.forEach((consent) => {
        const sigRef = consentSignatures.value[consent.key];

        if (sigRef && !sigRef.isEmpty()) {
            const base64 = sigRef.save('image/png');
            formData.consents[consent.key].signature = dataURLtoFile(
                base64,
                `${consent.key}_signature.png`,
            );
        } else {
    ...
    // Post directly using Inertia
    router.post('/api/submit-form', formData, {
        forceFormData: true,
        preserveScroll: true,
        onSuccess: () => {
